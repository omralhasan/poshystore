...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['signin'])) {
    ...
    if (empty($email) || empty($password)) {
        header('Location: signin.php?error=' . urlencode('Email and password are required'));
        exit();
    }
    
    // Check connection
    if (!$conn || $conn->connect_error) {
        header('Location: signin.php?error=' . urlencode('Database connection failed'));
        exit();
    }
    ...
    if (!$stmt) {
        $conn->close();
        header('Location: signin.php?error=' . urlencode('Database error'));
        exit();
    }
    ...
    if ($result->num_rows === 1) {
        ...
        if ($user['password'] === null || empty($user['password'])) {
            $stmt->close();
            $conn->close();
            $oauth_provider = ucfirst($user['oauth_provider'] ?? 'social media');
            header('Location: signin.php?error=' . urlencode("This account was created using {$oauth_provider}. Please sign in with {$oauth_provider}."));
            exit();
        }
...
<head>
    ...
    <title>Sign In - Poshy Store</title>
                    ...
                    <div class="card-ramadan p-4">
                        ...
                        <form action="signin.php" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="password" id="password" name="password" class="form-control-ramadan" required placeholder="Enter your password">
                            </div>

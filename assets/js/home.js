// Current language for translations
    const CURRENT_LANG = window.AppConfig.lang;
    const IS_LOGGED_IN = window.AppConfig.isLoggedIn;
    const CURRENCY_TEXT = window.AppConfig.currencyText;
    const ADD_TO_CART_TEXT = window.AppConfig.addToCartText;
    const LOGIN_TEXT = window.AppConfig.loginText;
    const DETAILS_TEXT = window.AppConfig.detailsText;
    const FEATURED_TEXT = window.AppConfig.featuredText;
    const VIEW_ALL_TEXT = window.AppConfig.viewAllText;
    const VIEW_ALL_LINK_TEXT = window.AppConfig.viewAllLinkText;
    const NO_PRODUCTS_TEXT = window.AppConfig.noProductsText;
    const TRY_SEARCHING_TEXT = window.AppConfig.trySearchingText;
    ...
    // ==========================================
    let currentFilter = window.AppConfig.currentFilter;
    ...
    function addToCart(productId, btn) {
        ...
        .then(data => {
            if (data.success) {
                ...
                showToast('success', window.AppConfig.toastAdded);
    ...
    runWhenIdle(function() {
        ...
        const LANG = window.AppConfig.lang;
        let timer     = null;
        let focused   = -1;
        let lastQuery = '';
        ...
        function showBox(items) {
            ...
            }).join('') + `<div class="sugg-footer">${window.AppConfig.searchFooter}</div>`;

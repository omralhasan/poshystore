...
<body>
    ...
    <!-- Category Hero -->
    <section class="category-hero" <?= !empty($current_category['image_path']) ? 'style="background: linear-gradient(135deg, rgba(26,26,46,0.85) 0%, rgba(15,52,96,0.95) 100%), url(\'../../' . htmlspecialchars($current_category['image_path']) . '\') center/cover;"' : '' ?>>
        <div class="category-hero-icon" <?= !empty($current_category['image_path']) ? 'style="background: transparent; box-shadow: none;"' : '' ?>>
            <?php if (!empty($current_category['image_path'])): ?>
                <img src="../../<?= htmlspecialchars($current_category['image_path']) ?>" alt="<?= htmlspecialchars($category_name) ?>" style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover; border: 3px solid #fff; box-shadow: 0 8px 25px rgba(197,160,89,0.3);">
            <?php else: ?>
                <i class="<?= $cat_icon ?>"></i>
            <?php endif; ?>
        </div>
        ...
        <?php else: ?>
            <div class="product-grid">
                <?php foreach ($products_array as $idx => $product): ?>
                    <?php 
                        $card_base_path = '../../';
                        include __DIR__ . '/../../includes/product_card_partial.php'; 
                    ?>
                <?php endforeach; ?>
            </div>
        <?php endif; ?>
